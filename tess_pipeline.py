...
from config import ensure_actor_dirs_exist
        ...
        for subdir, dirs, files in os.walk(path):
            for filename in files:
                if filename.startswith('OAF'):
                    destination_path = str(Path(TRAINING_FILES_PATH) / 'Actor_26') + os.sep
                    ...
                    destination_path = str(Path(TRAINING_FILES_PATH) / 'Actor_25') + os.sep
                    old_file_path = os.path.join(os.path.abspath(subdir), filename)
